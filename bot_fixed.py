#!/usr/bin/env python
# -*- coding: utf-8 -*-
# FINAL FIXED: NO [PENDING] ON FILLED, NO query_order() AFTER BUY
import requests
import hashlib
import hmac
import time
...
next_sell_check_time = int(time.time()) * 1000 - 30000
stopgainloss = False
global_pricediff = 0
theorderid = None
ispending = False
...
if status == "Filled":
    print(f"[INSTANT FILL] Order {theorderid} filled @ {od.get('FilledAverPrice')}")
    ispending = False
else:
    print(f"[PENDING] Order {theorderid} is {status}")
...
if __name__ == '__main__':
    print("TRADING BOT STARTED - FINAL FIXED - NO PENDING ON FILLED")
